...
def runDataflow(argv=None, save_main_session=True):
    ...
    def makeRow(e):
        return (
            e['user_id'], e['is_buy_completed'], e['is_fta_completed'], e['is_liked_completed'], e['is_list_completed'],
            e['is_profile_completed'], e['is_registeration_completed'], e['is_sold_completed'], e['is_ss_completed']
        )

    cols = ['user_id', 'is_buy_completed', 'is_fta_completed', 'is_liked_completed', 'is_list_completed',
            'is_profile_completed', 'is_registeration_completed', 'is_sold_completed', 'is_ss_completed']

    with beam.Pipeline(options=pipeline_options) as p:
        p | 'Collecting' >> beam.io.Read(beam.io.BigQuerySource(query=sql_stmt)) \
        | "Tuplify" >> beam.Map(makeRow) \
        | "Writing to spanner" >> beam.io.Write(SimpleKVSink(table_name="achieve",
                                                             project="kouzoh-p-bharath",
                                                             instance_id="test",
                                                             database_id="dummy",
                                                             columns=cols,
